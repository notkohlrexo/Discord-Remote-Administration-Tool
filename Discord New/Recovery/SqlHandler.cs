    ...
    internal class SqlHandler
    {
        ...
        private bool ReadTableFromOffset(ulong offset)
        {
            try
            {
                ...
                {
                    var num1 = (ushort) (ConvertToULong((int) offset + 3, 2) - 1UL);
                    var num2 = 0;
                    if (_tableEntries != null)
                    {
                        num2 = _tableEntries.Length;
                        Array.Resize(ref _tableEntries, _tableEntries.Length + num1 + 1);
                    }
                    else
                    {
                        _tableEntries = new TableEntry[num1 + 1];
                    }

                    for (ushort index1 = 0; (int) index1 <= (int) num1; ++index1)
                    {
                        var num3 = ConvertToULong((int) offset + 8 + index1 * 2, 2);
                        if ((long) offset != 100L)
                            num3 += offset;
                        ...
                        var array = (RecordHeaderField[]) null;
                                ...
                                if (array[index3].Type > 9L)
                                {
                                    if (!IsOdd(array[index3].Type))
                                    {
                                        if ((long) _dbEncoding == 1L)
                                            _tableEntries[num2 + index1].Content[index3] =
                ...
                {
                    var num1 = (ushort) (ConvertToULong((int) ((long) offset + 3L), 2) - 1UL);
                    for (ushort index = 0; (int) index <= (int) num1; ++index)
                    {
                        var num2 = (ushort) ConvertToULong((int) offset + 12 + index * 2, 2);
                ...
                switch (_fileBytes[offset])
                {
                    case 5:
                        ...
                        for (var index = 0; index <= (int) num1; ++index)
                        {
                            var num2 = (ushort) ConvertToULong((int) offset + 12 + index * 2, 2);
                            if (offset == 100L)
                                ReadMasterTable(((long) ConvertToULong(num2, 4) - 1L) * (long) _pageSize);
                            else
